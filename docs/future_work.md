...
以下事项已经结合当前 Halo Demo 页面确认有实际价值，可以按顺序实现；完成后按本文约定直接删除对应条目。

### 运维概览区分成功与降级成功

- **需要角色**：前端工程师、后端工程师。
- **Trigger**：任务最终完成、Job 和模型调用均显示成功，但子 Agent 失败或 fallback 参与，运维概览仍显示“近期失败 0、工作流成功率 100%”，无法识别非阻断异常。
- **DoD**：运维概览新增“降级执行”数量，并将失败 Agent、fallback、guardrail 等非阻断异常纳入“最近异常”；异常行能定位到对应任务和失败阶段。优先复用 `/api/v1/ops/metrics` 现有聚合，缺少任务级定位信息时再扩展后端响应；补聚合 / handler 测试，`cd backend && go test ./...` 与 `cd frontend && npm run build` 通过。

### 审核结果 CSV 导出

- **需要角色**：前端工程师、测试工程师。
- **Trigger**：测试工程师需要在表格中继续整理、评审或导入其他测试管理工具，现有内部 JSON 结构不便直接使用。
- **DoD**：任务审核页提供 CSV 导出，至少包含类别、标题、优先级、前置条件、步骤、预期结果、影响产品、影响模块和审核状态；正确处理逗号、双引号、换行与 UTF-8 BOM，导出范围与当前任务最终审核数据一致；JSON 导出继续保留。将 CSV 序列化拆为可测试工具函数，补边界测试并保证 `cd frontend && npm run build` 通过。

### 知识建议证据前置

- **需要角色**：前端工程师、测试工程师。
- **Trigger**：候选名称可能是仓库名、普通缩写或其他低质量词条，但页面允许用户在未查看出现明细时直接采纳。
- **DoD**：知识建议表格在“采纳”之前提供明确的“查看来源”入口，展示来源任务、用例、出现频次和上下文片段；采纳流程允许先修正候选类型和名称，再生成知识草稿，不再依赖最右侧无文字的展开图标发现证据。现有采纳、忽略和自动过期流程保持可用，`cd frontend && npm run build` 通过。

### 中文日期时间格式统一

- **需要角色**：前端工程师。
- **Trigger**：当前 `Intl.DateTimeFormat` 使用浏览器默认 locale，中文界面会因运行环境不同显示 `06/22/2026`、`22/06/2026` 等不同顺序。
- **DoD**：`frontend/src/utils/date.js` 固定输出无歧义的本地时间格式 `YYYY-MM-DD HH:mm:ss`，所有现有页面继续复用同一工具；无效值和空值仍返回 fallback。补格式化工具测试或等价可机器复核验证，`cd frontend && npm run build` 通过。
