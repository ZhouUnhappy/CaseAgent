...
- **需要角色**：前端工程师。
- **Trigger**：常规桌面宽度下，生成工作台顶部连续展示页面指标、项目摘要和流程状态三层信息，项目名称与输入来源重复出现；结果区工具栏挤压标题，刷新 / 审核等操作重复，影响主流程识别。
- **DoD**：在 `frontend/src/views/CaseGenerationWorkspace.vue` 合并项目选择与三步流程状态，删除项目描述、顶部重复指标和重复动态主操作；文档上传、任务创建 / 生成、用例审核等操作只保留在对应步骤，结果区明确审核为主操作、导出为次操作；来源统一显示为“参考文档”和“知识库依据”；消除流程条空白列、常规桌面宽度标题换行及结果区内外双重滚动。`cd frontend && npm run build` 通过，并使用 Halo 缩略图 Demo 在常规桌面宽度验证项目切换、任务状态、来源依据和用例输出可见。
