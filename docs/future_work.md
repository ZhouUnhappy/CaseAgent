...
以下事项已由真实 Halo 任务（4 个类别、41 条用例）的人工审核体验触发，按顺序完成；它们都是一次性优化，完成并验证后从本节删除。

### 审核正确性

**需要角色**：前端工程师、后端工程师、测试工程师。

**Trigger**：行级勾选实际提交整个类别，且工作台当前文档选择与已生成任务输入可以不一致；41 条用例也无法显示已审核进度。条件已满足。

**DoD**：类别提交使用独立的类别选择，不再复用行选择；结果区明确展示任务输入文档并提示当前选择差异；复用现有 feedback 数据提供逐条“通过 / 有问题”、审核进度和待审核筛选；后端现有类别提交与 feedback 测试继续通过。

### 审核效率与依据阅读

**需要角色**：前端工程师、测试工程师。

**Trigger**：任务详情同时平铺审核和 Agent 诊断数据，真实任务需要在 41 条用例中定位标题并反复查看需求依据。条件已满足。

**DoD**：任务详情默认展示“用例审核”，技术诊断切换后才展示；“调试”改为“查看依据”，首先展示需求和知识片段，Agent / Model 信息进入技术详情；支持标题、前置条件和步骤关键词搜索；标题及审核操作在桌面表格横向滚动时保持可见。

### 前端范围收敛

**需要角色**：前端工程师、测试工程师。

**Trigger**：知识库主界面已经暴露有效期、重复指向、影响任务和后台 job 等当前计划明确不启动的治理能力，干扰预览与编辑主流程。条件已满足。

**DoD**：知识库主界面只保留类型、名称、处理状态、更新时间及预览 / 编辑主操作；重新处理和删除收入口菜单；隐藏当前不需要的有效期、重复、影响任务、来源筛选和高级表单字段，不改变现有后端数据兼容性。

### 文档预览完整性

**需要角色**：前端工程师、测试工程师。

**Trigger**：现有自定义 Markdown 子集会丢失粗体、行内代码、链接和表格，无法可靠预览常见需求文档。条件已满足。

**DoD**：改用成熟 Markdown 解析器支持 GFM 表格、链接、代码和常用行内格式；渲染 HTML 必须经过清洗；需求文档与知识预览共用同一组件；前端生产构建通过。
