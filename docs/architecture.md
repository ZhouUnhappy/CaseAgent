...
**做什么**：分析阶段自动从需求里挖未覆盖的"产品/模块"候选，写入 `knowledge_update_suggestions`；用户也可以在用例审核页手动反馈知识缺失。前端列表支持采纳（生成知识条目草稿并跳到知识库页，保存后回填 `resolved_knowledge_id`）/ 忽略；长期未处理的 pending 建议会自动过期为 `dismissed_reason='auto_expired'`。
