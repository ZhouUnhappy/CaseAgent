...
- 关键 helper：
  ...
  - `handler.RunAsyncAfterCommitWithFailure(c, h.DB, tenantID, fn, onError)` — 当异步任务失败后还需要持久化失败状态 / suggestion 时使用；`fn` 事务回滚后，`onError` 会在新的 tenant tx 里运行
...
4. **handler**：所有 INSERT 路径填 `TenantID: handler.TenantIDFromContext(c)`；service.New 接 `bun.IDB`；异步任务用 `handler.RunAsyncAfterCommit`；异步失败需要落库时用 `handler.RunAsyncAfterCommitWithFailure`
