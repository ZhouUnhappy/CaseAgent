...
- [ ] 4.1.1 新建 `backend/internal/api/middleware/tenant.go`：从 `X-Tenant-ID` header 解析 → 查 `tenants` 表存在性 → 注入 `gin.Context`（key: `tenant_id`）
- [ ] 4.1.2 缺失或不存在时返回 400，但**白名单 `/api/v1/tenants`**（建租户本身不需要 tenant 上下文）
- [ ] 4.1.3 中间件单测：合法 / 缺失 / 不存在 / 非法格式 四种 case

### 4.2 事务中间件

- [ ] 4.2.1 新建 `middleware/tx.go`：在 tenant 中间件之后，对每个请求开事务，把事务对象塞进 `gin.Context`（key: `db`），请求结束 commit/rollback
- [ ] 4.2.2 提供 `handler.DBFromContext(c) bun.IDB` 辅助
- [ ] 4.2.3 注意：异步 goroutine（如 `handler/document.go:89` 的 `go func()`）**不能复用请求事务**，要单独开 —— handler 里抽 `runInTenantTx(tenantID, fn)` 帮手

### 4.3 路由注册中间件

- [ ] 4.3.1 `router/router.go:13`：`v1.Use(middleware.Tenant(db), middleware.Tx(db))`
- [ ] 4.3.2 `tenants` 路由组单独注册（不挂 tenant 中间件，只挂 tx）
...
- [ ] 6.1.1 `service/retrieval/service.go:174` `SearchKnowledge`：不需要手动加 WHERE（RLS 兜底），但要确认走的是事务内的 IDB
- [ ] 6.1.2 `db/pgvector/retriever.go` 同上
- [ ] 6.1.3 写集成测试：两个租户各塞 10 条知识，分别检索验证只见到自己的（确认 RLS 隔离）

### 6.2 hnsw 索引性能验证

- [ ] 6.2.1 跑现有 `scripts/i1_retrieval_smoke.sh` 确认基线
- [ ] 6.2.2 用脚本造 1k 条知识 × 5 个租户的数据，对比 RLS 过滤前后的查询延迟（应在 50ms 以内）
- [ ] 6.2.3 如果延迟超标，调 hnsw 的 `ef_search` 或考虑 per-tenant partial index
...
- [ ] 8.1.1 新建 `frontend/src/stores/tenant.js`：当前 tenant slug 持久化到 localStorage
- [ ] 8.1.2 `frontend/src/api/client.js:9` 拦截器加 `X-Tenant-ID` header
- [ ] 8.1.3 `frontend/src/layout/AppLayout.vue` 加 tenant 选择器（顶栏下拉）
- [ ] 8.1.4 切换 tenant 后 reload 当前路由数据
...
- [ ] 9.1.1 所有 `scripts/i*.sh` curl 调用统一加 `-H "X-Tenant-ID: $TENANT"`，按上面矩阵设默认
- [ ] 9.1.2 `i1_private_corpus_eval.sh`：缺 `CASEAGENT_I1_PRIVATE_TENANT_SLUG` 时立即 exit 非 0，错误信息说明原因
- [ ] 9.1.3 `i1_private_corpus_eval.sh` 末尾追加反向断言：切到 `i1-smoke` tenant 检索同一查询，必须返回空（验证私有数据不漏到其他 tenant）
- [ ] 9.1.4 `i1_retrieval_determinism.sh` / `i2_generation_e2e.sh`：从被复用 doc/project 自动回查 `tenant_id` 并注入 header
- [ ] 9.1.5 `i1_retrieval_cleanup.sh`：保持 superuser DSN，文档注明它绕过 RLS（design intent）
- [ ] 9.1.6 在 `scripts/README.md` 顶部加入"租户分配原则"段落

### 9.2 多租户隔离专项脚本

- [ ] 9.2.1 新增 `scripts/multitenancy_isolation.sh`：建两个对等 tenant（A 和 B），用同一份 smoke fixture 各自上传到自己 tenant，互相检索对方 fixture 必须返回空（验证"私有 vs 私有"语义，与 9.1.3 的"私有 vs 跨 tenant"互补）

### 9.3 单测

- [ ] 9.3.1 `backend/...` 现有 `*_test.go` 修复（model 字段增加、handler 签名变化）
- [ ] 9.3.2 新增 `backend/internal/db/schema_rls_test.go`（在 3.1.4 完成）
- [ ] 9.3.3 新增 `backend/internal/api/middleware/tenant_test.go`（在 4.1.3 完成）
