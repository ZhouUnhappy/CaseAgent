...
| --- | --- | --- | --- | --- |
| I1-T1 | 文档链路真实联调（上传 -> 分块 -> embedding -> 检索） | In Progress | 在具备后端、PostgreSQL、pgvector、embedding 配置的环境下，`bash scripts/i1_retrieval_smoke.sh` 连续 3 次执行均：（a）通过 run token 或本轮 `document_ids` 与历史数据隔离；（b）文档状态最终为 `completed`、分块与 embedding 数量 > 0；（c）`top_k=5` 检索结果中本轮上传文档排名第一。 | 已补 `testdata/i1/requirement.md` 与 `scripts/i1_retrieval_smoke.sh`；smoke 已用 `document_ids` 限定并断言 rank-1 命中本轮 `document_id`、rank-1 entry `matched_chunks` 非空；启用 `CASEAGENT_PSQL_DSN` 时校验 `document_chunks` 行数 > 0 且 embedding 全非空；后端启动会补齐老库 `documents.content`。 |
| I1-T2 | 知识库链路真实联调（创建 -> embedding -> 检索） | In Progress | 在具备后端、PostgreSQL、pgvector、embedding 配置的环境下，`bash scripts/i1_retrieval_smoke.sh` 连续 3 次执行均：（a）通过 run token 或本轮创建对象 ID/metadata 与历史数据隔离；（b）知识条目状态最终为 `completed`、embedding 已写入；（c）`top_k=5` 检索结果中本轮创建知识条目排名第一。 | 已补 `testdata/i1/product_knowledge.md`、`testdata/i1/module_knowledge.md` 与 smoke 脚本；smoke 每次生成 `RUN_TOKEN` 写入知识 `metadata.run_token`，断言 rank-1 同时匹配本轮 `module_knowledge_id` 与 `metadata.run_token`；启用 `CASEAGENT_PSQL_DSN` 时校验本轮 `knowledge_base` 行 embedding 非空；已修复老库 `knowledge_base.status/metadata/created_at/updated_at` 补列。 |
| I1-T3 | 检索回归样例沉淀 | In Progress | 在仓库内（建议 `docs/regression/i1_retrieval.md`）提交至少 2 条回归样例（文档/知识库各 1 条），每条包含：fixture 路径、查询词、期望命中对象、执行命令、前置环境、最近一次实际结果摘要；T1/T2 每次稳定通过后更新摘要。 | 已提交 `docs/regression/i1_retrieval.md`，含文档/知识库各 1 条样例骨架（fixture/查询词/期望命中/执行命令/前置环境/复现流程）；最近一次实际结果摘要待 T1/T2 首次连续 3 次通过后回填。 |
