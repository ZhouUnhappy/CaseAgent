...
| I1-T3 | 历史异维度向量治理 | In Progress | 清理/重建后在 `2000` 维配置下无维度冲突错误。 | 已有向量健康检查与批量重建入口。 |
| I1-T4 | 检索回归样例沉淀 | Todo | 至少 2 条可复现回归样例（文档/知识库各 1 条）。 | 待补充。 |

### 当前已完成能力（支撑 I1）

- 后端基础结构已落地：配置加载、Gin 路由、Bun 模型、迁移脚本、服务层骨架。
- pgvector 自定义向量编码/解码已完成，支持按配置维度自动校正向量列。
